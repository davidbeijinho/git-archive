...
IRsend irsend(D0);  // An IR LED is controlled by GPIO pin 14 (D5)
...
{
  MQTT.loop();
  // Serial.println("HELLO FROM SETUP 1"); // prints a label
  // blinkLed();
  IR.printData();
  //BMP.printData();
  loopLG();
...
//const myIr = require("myIr");
// const myBpm = require("myBpm");
// const myLed = require("myLed");
// const myWifi = require("myWifi");
// const myServer = require("myServer");
//const UTILS = require("myUtils");

// function afterWifi(error) {
//   if (error) {
//     console.log("ERROR CONNECTING", error);
//     return;
//   }
//   {
//     myServer.createServer(config.PORT);
//   }
// }

//E.on("init", function() {
// UTILS.setStartTime(Date.now());
// console.log("INFO: Start Time", UTILS.getUptime());
// console.log("config",config);
// console.log("connect", myIr.connect);
// console.log("setFunction", myIr.setFunction);
// console.log("removeFunction", myIr.removeFunction);

// myIr.connect(config.IR_PIN);
// myBpm.connect(config.BPM_PINS);
// myLed.connect(config.LED_PINS, config.LED_INIT);
// myWifi.connect(config.WIFI, afterWifi);
//});
